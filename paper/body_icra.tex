...
speed-versus-smoothness selector. The DI
...
\(\kappa=(\dot x\ddot y-\dot y\ddot x)/(\dot x^2+\dot y^2)^{3/2}\). The
benchmark evaluates one concrete claim: applying the constant-\(A_d\)
principle to the lateral channel---optimizing lateral jerk, the
derivative that sets steering rate---produces a steering profile inside a
representative actuator-rate envelope where the path-first command does not.
On a lane-change maneuver at a fixed \(13\)\,m/s cruise speed with a
0.5\,rad/s steering-rate cap, the jerk-bounded time-first extension stays
within the cap (peak rate 0.130\,rad/s) while the path-first command
exceeds it on the double lane change (0.594\,rad/s) because
circular-blend curvature steps at path seams; since steering rate scales
with speed at fixed path curvature, this margin is specific to that
operating point. The result is a lateral-channel claim at a fixed cruise
speed, complete and tested for that channel; vehicle feasibility beyond
it comes from speed, acceleration, curvature, and steering-rate
constraints on top of the parameterization.
...
replan latency, TOTG and DI have comparable completion times.
...
\(10\times\) lower mean compute; bounded-latency reactivity is thus shared
with MPPI, and pairing it with hard limit feasibility at
QP-level cost is what the DI-QP adds.
...
samples parallel rollouts. A parallel line keeps a precomputed path but
governs the reference passed downstream: the Path Feasibility Governor
\cite{pathfg} selects an auxiliary reference along the path that a
nonlinear MPC is guaranteed to track with recursive feasibility. The
present planner modifies the reference in the same spirit but dispenses
with the path altogether, trading that formal certificate for a
fixed-structure convex QP and configuration-independent precomputation.
The present method shares this ``reactivity first'' philosophy but
reaches it through a convex QP
