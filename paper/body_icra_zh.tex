...
\(\kappa=(\dot x\ddot y-\dot y\ddot x)/(\dot x^2+\dot y^2)^{3/2}\) 恢复航向角与曲率。这只是一个初步的示例，而不是一个完整的自行车运动学模型规划器：在一个固定 \(13\)\,m/s 巡航速度、具有代表性的 0.5\,rad/s 转向速率上限的变道机动中，加加速度受约束的时间优先扩展方法始终保持在该上限之内（峰值速率 0.130\,rad/s），而路径优先指令在双移线动作中超出了该上限（0.594\,rad/s），原因是圆弧过渡曲率在路径接缝处发生阶跃；由于在固定路径曲率下转向速率会随车速缩放，这一裕量仅针对该工作点而言，并非对所有速度下均成立的结论。完整的自行车模型处理留待未来工作。
